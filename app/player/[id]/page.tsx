...
export default function Page() {
    ...
    useEffect(() => {
        ...
        fetch(`https://us-central1-summaristt.cloudfunctions.net/getBook?id=${id}`)
            ...
            .then((data) => {
                console.log("API data:", data); // optional, for debugging
                if (data && typeof data === 'object') {
                    setPlayer([data]); // Wrap the object in an array
                    ...
                    <div className="summary">
                        ...
                        {Array.isArray(filteredPlayer) && filteredPlayer.length > 0 ? (
                            filteredPlayer.map((player) => (
                                <div key={player.id} className="audio__book--summary" style={{ fontSize: "16px" }}>
                                    <div className="audio__book--summary-title"><b>{player.title}</b></div>
                                    <div className="audio__book--summary-text">{player.summary}</div>
                                    <div className="audio__wrapper">
                                        <div className="audio__track--wrapper">
                                            <figure className="audio__track--image-mask" style={{ width: "48px", height: "48px", minWidth: "48px" }}>
                                        ...
                                        <div className="audio__controls--wrapper">
                                            <audio
                                                ...
                                                onTimeUpdate={(e) => handleAudioTimeUpdate(e.target as HTMLAudioElement)}
                                            ...
                                            <div className="audio__controls">
                                                ...
                                                <button
                                                    ...
                                                    title="Previous"
                                                    onClick={() => {
                                                        const audioElement = document.getElementById(`audio-player-${player.id}`) as HTMLAudioElement;
                                                        if (audioElement) audioElement.currentTime = Math.max(0, audioElement.currentTime - 10); // Skip back 10 seconds
                                                    }}
                                                >
                                                    <RiReplay10Fill />
                                                </button>
                                                ...
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        const audioElement = document.getElementById(`audio-player-${player.id}`) as HTMLAudioElement;
                                                        if (audioElement) audioElement.currentTime = Math.min(audioElement.duration, audioElement.currentTime + 10); // Skip forward 10 seconds
                                                    }}
                                                >
                                                    <RiForward10Fill />
                                                </button>
                                        ...
                                        <div className="audio__progress--wrapper">
                                            ...
                                            <label htmlFor={`audio-progress-${player.id}`} className="visually-hidden"></label>
                                            <input
                                                type="range"
                                                ...
                                                onChange={(e) => {
                                                    const audioElement = document.getElementById(`audio-player-${player.id}`) as HTMLAudioElement;
                                                    if (audioElement) audioElement.currentTime = parseFloat(e.target.value);
